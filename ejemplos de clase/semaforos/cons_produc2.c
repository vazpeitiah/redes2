/*
  Descripcion: programa que tiene 2 productores y un consumidor
...
void *func_prod1(void *arg)
{
  ...
  {  
    sem_wait(&sem_pro);
    cont++;
     printf("p2 %d: se produjo 0 1 2 \t cont = %d \n", i+1, cont);
    sem_post(&sem_con);
  }
}
...
void *func_prod2(void *arg)
{
  ...
  {  
    sem_wait(&sem_pro);
    cont++;
    printf("p1 %d: se produjo 3 4 5 \t cont = %d \n", i+1, cont);
    sem_post(&sem_con);
  }
}
...
void *func_cons(void *arg)
{
  for (int i = 0; i < 40; i++) 
  {
    sem_wait(&sem_con);
    cont--;
    printf("se consumio un item \t cont = %d \n", cont);
    sem_post(&sem_pro);
  }
}
